...
namespace System.CommandLine.Hosting.Tests
{
    public static class HostingTests
    {
        ...
        [Fact]
        public static void UseHost_can_call_inner_method_with_constructor_injection()
        {
            const int myValue = 4224;
            string commandLine = $"-{nameof(MyOptions.MyArgument)} {myValue}";
            ...
            rootCmd.AssignHandler<CommandExecuter>();
            ...
            int result = new CommandLineBuilder(rootCmd)
                .UseHost(host =>
                {
                    host.ConfigureServices(services =>
                    {
                        ...
                        services.AddTransient<CommandExecuter>();
                    });
                })
                .Build()
                .Invoke(commandLine);
            ...
            Assert.Equal(0, result);
            Assert.Equal(myValue, service.SomeValue);
        }

        [Fact]
        public static void UseHost_can_call_inner_method_with_constructor_injection_on_subcommand()
        {
            const int myValue = 4224;
            string commandLine = $"--subCommand -{nameof(MyOptions.MyArgument)} {myValue}";
            var service = new MyService()
            {
                SomeValue = -1
            };
            ...
            subCommand.AssignHandler<CommandExecuter>();
